# Gemini Notebook Prompt Queue

A small Chrome extension that queues up prompts for [Gemini Notebook](https://notebook.google.com) and auto-submits them one after the other. Send 20 prompts before your morning coffee, come back to 20 answers.

Gemini Notebook has no public API, so this is the next best thing: it drives the chat UI directly via a content script.
...
- **All lookups are fresh queries** — no held references — so Gemini Notebook re-rendering the chat area between prompts doesn't break anything.

If submission breaks because Gemini Notebook changes its UI, the fix is editing the `CHAT_PATTERNS` / `EXCLUDED_PATTERNS` arrays at the top of `content.js`.
...
- Gemini Notebook tab needs to stay visible. Chrome throttles JS timers in background tabs.
- Doesn't capture responses — they stay in Gemini Notebook as normal chat messages.
- If Gemini Notebook significantly redesigns the chat UI, selectors may need an update.

## Debug

Open DevTools on the Notebook tab and watch the console for `[NBLM Queue]` log lines. Every step logs what it's doing.
...
| `content.js` | Runs on Gemini Notebook — finds inputs, submits, waits for completion |
...
- Selector fixes when Gemini Notebook updates its UI
